 ...
 *
 * Auth (either; no credentials → setup instructions + exit 1, never fake data):
 *   1. GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json
 *      The SA must be added as a user on the GSC property (Settings → Users):
 *      known SA on this machine: firebase-adminsdk-tjtuk@fdata-299302.iam.gserviceaccount.com
 *      (key at ~/Documents/fdata/fdata-299302-new-key.json)
 *   2. GSC_TOKEN_FILE=/path/to/token.json  with {"access_token":"ya29..."}
 *
...
async function accessToken() {
  ...
  try {
    ...
    console.error('[measure-gsc] no credentials. Either set GOOGLE_APPLICATION_CREDENTIALS');
    console.error('to a service-account key that is a user on the GSC property, or GSC_TOKEN_FILE.');
    console.error('One-time setup: GSC → Settings → Users → add the SA email as Full user.');
    process.exit(1);
  }
...
try {
  ...
  if (String(e.message).includes('403')) {
    console.error('\n403 = this credential is not a user on the GSC property.');
    console.error('One-time fix (user action, ~1 min): Search Console → forest-international.com');
    console.error('→ Settings → Users and permissions → Add user →');
    console.error('  firebase-adminsdk-tjtuk@fdata-299302.iam.gserviceaccount.com  (Full)');
    console.error('Then set gsc.api_ready=true in seo/_registry.json.');
  }
  process.exit(1);
}
