...
6. **Publish = full indexing loop, or it doesn't count:** build passes → push → verify the change is LIVE on the production URL (deploy pipeline: see skill `references/publishing.md` §3 — not confirmed automatic) → sitemap contains it → IndexNow ping (Bing = ChatGPT retrieval) → GSC inspection at T+7. Never index-ping a URL that isn't live.
...
The SEO loop (`/school-seo-loop`) runs autonomously in THIS repo: once the quality gate fully passes it may commit, push origin main, verify live, ping IndexNow, and update state files without per-action approval (production-deploy trigger: publishing.md §3). In exchange:
